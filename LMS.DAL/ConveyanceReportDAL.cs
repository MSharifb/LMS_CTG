...
namespace LMS.DAL
{
    public class ConveyanceReportDAL
    {
        public static List<ConveyanceReport> GetReportData(string strEmpID, string strDepartmentID, string strFromDate, string strToDate, int startIndex, int rowNumber, out int numTotalRows)
        {
            try
            {
                ...
                DataTable dt = db.ExecuteQueryToDataTable(cpList.ParameterNames, cpList.ParameterValues, cpList.ParameterTypes, ref paramval, null, "OOA_USPCONVEYANCEREPORTGET");
                numTotalRows = (int)paramval;
                List<ConveyanceReport> results = new List<ConveyanceReport>();
                foreach (DataRow dr in dt.Rows)
                {
                    ConveyanceReport obj = new ConveyanceReport();
