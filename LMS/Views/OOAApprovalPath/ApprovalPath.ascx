...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $("#divStyleApproval").dialog({ autoOpen: false, modal: true, height: 550, width: 820, resizable: false, title: 'Approval Flow',
            close: function (ev, ui) {
                var pg = $('#txtPageNo').val();

                var targetDiv = "#divDataList";
                var url = '/LMS/OOAApprovalPath/ApprovalPath?page=' + pg;
    ...
    function deleteApprovalPath(Id) {
        ...
        if (result == true) {
            ...
            executeCustomAction({ intApprovalPathID: Id }, '/LMS/OOAApprovalPath/Delete', 'divDataList');
        }
        return false;
    }
    ...
    function openSetApprover(Id) {
        ...
        var host = window.location.host;
        var url = 'http://' + host + '/LMS/OOAApprovalPath/SetApprover/' + Id;
    ...
    function popupStyleDetails(Id) {
        var host = window.location.host;
        var url = 'http://' + host + '/LMS/OOAApprovalPath/Details/' + Id;
    ...
    function popupStyleAdd() {
        ...
        var host = window.location.host;
        var url = 'http://' + host + '/LMS/OOAApprovalPath/ApprovalPathAdd';
        $('#styleOpenerApproval').attr({ src: url });
        $("#divStyleApproval").dialog('open');
        return false;
    }
    ...
    function searchData() {
        var targetDiv = "#divDataList";
        var url = "/LMS/OOAApprovalPath/ApprovalPath";
        var form = $("#frmApprovalPath");
        var serializedForm = form.serialize();
        $.post(url, serializedForm, function (result) { $(targetDiv).html(result); }, "html");
...
<div id="grid">
    <div id="grid-data" style="overflow: auto; width: 99%">
        <table class="contenttext" style="width: 100%;">
            ...
            <tbody>
                <%  if (Model.LstOOAApprovalPathMaster1 != null) foreach (LMSEntity.OOAApprovalPathMaster obj in Model.LstOOAApprovalPathMaster1)
                        { 
                %>
                <tr>
                    <td style="width: 80%;">
    ...
    <div class="pager">
        <%= Html.PagerWithScript(ViewData.Model.LstOOAApprovalPathMaster1.PageSize, ViewData.Model.LstOOAApprovalPathMaster1.PageNumber, ViewData.Model.LstOOAApprovalPathMaster1.TotalItemCount, "frmApprovalPath", "/LMS/OOAApprovalPath/ApprovalPath", "divDataList")%>
        <%= Html.Hidden("txtPageNo", ViewData.Model.LstOOAApprovalPathMaster1.PageNumber)%>
    </div>
    <label id="lblTotalRows">
        Total Records:<%=Model.LstOOAApprovalPathMaster.Count.ToString()%></label>
</div>
<div class="spacer">
</div>
</form>
